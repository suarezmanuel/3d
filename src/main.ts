...
import { draw2d_triangle, draw_textured_triangle, fill2d_triangle } from './shapes/drawing.js';
...
function updatefps (elapsedTime: number, intervalMs: number, lastTime: number) {
...
export class Scene {
    ...
    private vLight = new vec3d(0,0,-1);
    ...
    async IntializeTextureSampler() {
        await this.sampler.init_sampler("../files/minecraft_0.png");
    }
    ...
    render() {
        ...
        this.objects.forEach((o: mesh) => {
            o.tris.forEach((tri: triangle3d) => {
                ...
                if (normal.dot_product(camRay) < 0) {
                    ...
                    for (let n=0; n < clipped_triangle_count; n++) {
                        ...
                        triProjected.t[0].u = clipped[n].t[0].u; triProjected.t[0].v = clipped[n].t[0].v; triProjected.t[0].w = clipped[n].t[0].w;
                        triProjected.t[1].u = clipped[n].t[1].u; triProjected.t[1].v = clipped[n].t[1].v; triProjected.t[1].w = clipped[n].t[1].w;
                        triProjected.t[2].u = clipped[n].t[2].u; triProjected.t[2].v = clipped[n].t[2].v; triProjected.t[2].w = clipped[n].t[2].w;
        ...
        trisToRaster.forEach((tri: triangle3d) => {
            ...
            listTriangles.forEach((tri: triangle3d) => {
                ...
                draw_textured_triangle (tri.p[0].v[0], tri.p[0].v[1], tri.t[0].u, tri.t[0].v, tri.t[0].w,
                                        ...
                                        tri.p[2].v[0], tri.p[2].v[1], tri.t[2].u, tri.t[2].v, tri.t[2].w, this.sampler, this.ctx, this.depthBuffer);
    ...
    async start () {
        try {
            await this.IntializeTextureSampler();
            this.init();
        } catch (error) {
            console.error(error)
        }
